...
def parse_nl_query(query: str) -> dict:
    ...
    # Gender matching
    has_male = any(
        re.search(r"\b" + kw + r"\b", query) for kw in GENDER_KEYWORDS["male"]
    )
    has_female = any(
        re.search(r"\b" + kw + r"\b", query)
    ...
    pattern = (
        r"\bfrom\s+([a-z]+(?:\s+[a-z]+)*)"
        r"(?=\s+(?:aged?|above|over|below|under|between|who|that|with)\b|\s*$)"
    )
    ...
    # Match "Above age x"
    pattern = (
        r"\b(?:above|over|older than|greater than)"
        r"(?:\s+the)?(?:\s+age)?(?:\s+of)?\s+(\d+)\b"
    )
    ...
    # Match "Below age x"
    pattern = (
        r"\b(?:below|under|younger than|less than)"
        r"(?:\s+the)?(?:\s+age)?(?:\s+of)?\s+(\d+)\b"
    )
